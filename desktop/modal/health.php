...
$eqLogics = extel::byType('extel');
?>

<table class="table table-condensed tablesorter" id="table_healthextel">
	<thead>
		<tr>
			<th>{{Image}}</th>
			<th>{{Module}}</th>
...
foreach ($eqLogics as $eqLogic) {
    ...
    if ($eqLogic->getConfiguration('model', '') != '') {
        $image = '<img src="plugins/extel/doc/images/' . $eqLogic->getConfiguration('model', '') . '.jpg" height="55" width="55" />';
    } else {
        $image = '<img src="plugins/extel/doc/images/extel_icon.png" height="55" width="55" />';
    }
